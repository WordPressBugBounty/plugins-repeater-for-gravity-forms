(function ($) {
	...
	jQuery(document).ready(function ($) {
		...
		gform.addFilter('gform_is_value_match', function (isMatch, formId, rule) {
			var check_repeater = rule['fieldId'].toString().split("-");
			if (check_repeater.length < 2) {
				return isMatch;
			} else {
				var $ = jQuery,
					inputId = rule['fieldId'],
					baseInputId = check_repeater[0],
					randId = check_repeater[1],
					baseFieldId = parseInt(baseInputId, 10),
					inputIndex = gformExtractInputIndex(baseInputId),
					isInputSpecific = inputIndex !== false,
					$inputs;
				if (isInputSpecific) {
					$inputs = $('#input_{0}_{1}_{2}-{3}, #choice_{0}_{1}_{2}-{3}'.gformFormat(formId, baseFieldId, inputIndex, randId));
				} else {
					$inputs = $('input[id="input_{0}_{1}-{2}"], input[id^="input_{0}_{1}_"][id$="-{2}"], input[id^="choice_{0}_{1}_"][id$="-{2}"], select#input_{0}_{1}-{2}, textarea#input_{0}_{1}-{2}'.gformFormat(formId, baseFieldId, randId));
				}
				var isCheckable = $.inArray($inputs.attr('type'), ['checkbox', 'radio']) !== -1;
				var isMatch = isCheckable ? gf_is_match_checkable($inputs, rule, formId, inputId) : gf_is_match_default($inputs.eq(0), rule, formId, inputId);
				return isMatch;
			}
		});
		...
		function yeeaddons_gf_get_input_id_by_html_id(htmlId) {
			...
			if (ids.length >= 3) {
				...
				if (type === 'choice') {
					var fieldId = ids[2];
					var randSuffix = '';
					var lastPart = ids[ids.length - 1];
					if (lastPart && lastPart.indexOf('-') !== -1) {
						var parts = lastPart.split('-');
						randSuffix = '-' + parts[1];
					}
					return fieldId + randSuffix;
				} else if (type === 'input') {
					if (ids.length === 3) {
						return ids[2];
